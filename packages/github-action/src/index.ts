...
import { buildMarkdownReport, filterResultForChanges } from "./report.js";
...
async function changedFiles(root: string, base: string): Promise<string[]> {
  ...
  const { stdout } = await execFileAsync("git", ["diff", "--name-only", "--diff-filter=ACDMR", `${base}...HEAD`, "--"], { cwd: root, encoding: "utf8", maxBuffer: 2_000_000 });
